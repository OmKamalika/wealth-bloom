...
export interface CurrencyInfo {
  ...
  locale: string;     // Locale for number formatting (e.g., "en-US", "en-IN")
  name: string;       // Currency name (e.g., "US Dollar", "Indian Rupee")
  position: 'prefix' | 'suffix'; // Whether symbol appears before or after the number
}
...
export const countryCallingCodeMap: Record<string, string> = {
...
export function getCurrencyInfoFromCountry(countryCode: string): CurrencyInfo {
  return countryCurrencyMap[countryCode] || defaultCurrency;
}

// Get currency info from calling code
export function getCurrencyInfoFromCallingCode(callingCode: string): CurrencyInfo {
  const countryCode = countryCallingCodeMap[callingCode];
  return countryCode ? getCurrencyInfoFromCountry(countryCode) : defaultCurrency;
}