...
export class AdvancedWealthCalculatorScenarios {
  ...
  private static readonly SCENARIO_PROBABILITIES = {
  ...
  private static readonly MARKET_RETURN_ADJUSTMENTS: Record<string, number> = {
  ...
  private static readonly INFLATION_ADJUSTMENTS: Record<string, number> = {
  ...
  private static readonly LIFE_EVENT_ADJUSTMENTS: Record<string, number> = {
   ...
   */
  static generateAlternativeScenarios(
    ...
    scenarios.bestCase = this.generateScenario(
      inputs,
      baseProjections,
      'bestCase'
    );
    ...
    scenarios.worstCase = this.generateScenario(
      inputs,
      baseProjections,
      'worstCase'
    );
    ...
    scenarios.stressTest = this.generateScenario(
      inputs,
      baseProjections,
      'stressTest'
    );
  ...
  private static generateScenario(
    inputs: CalculatorData,
    baseProjections: WealthProjection[],
    scenarioType: 'bestCase' | 'mostLikely' | 'worstCase' | 'stressTest'
  ): any {
    ...
    const adjustedProjections = this.adjustProjectionsForScenario(
      baseProjections,
      scenarioType
    );
   ...
   */
  private static adjustInputsForScenario(
    inputs: CalculatorData,
    scenarioType: 'bestCase' | 'mostLikely' | 'worstCase' | 'stressTest'
  ): CalculatorData {
    // Create a deep copy of inputs
    const adjustedInputs = JSON.parse(JSON.stringify(inputs)) as CalculatorData;
   ...
   */
  private static adjustProjectionsForScenario(
    baseProjections: WealthProjection[],
    scenarioType: 'bestCase' | 'mostLikely' | 'worstCase' | 'stressTest'
  ): WealthProjection[] {
    // Create a deep copy of projections
    const adjustedProjections = JSON.parse(JSON.stringify(baseProjections)) as WealthProjection[];
   ...
   */
  private static findExtinctionYear(projections: WealthProjection[]): number {
   ...
   */
  private static getScenarioDescription(scenarioType: string): string {
    switch (scenarioType) {
      case 'bestCase':
   ...
   */
  static generateWhatIfScenarios(inputs: CalculatorData): Array<{
    name: string;
    description: string;
