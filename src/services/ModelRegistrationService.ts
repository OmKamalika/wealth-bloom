...
import {
  ...
  ModelMonitoringPlan,
  ModelDocumentation
} from '../types/model-risk-management';
...
export class ModelRegistrationService {
  ...
  private validationStrategies: Map<string, ModelValidationStrategy> = new Map();
  private monitoringPlans: Map<string, ModelMonitoringPlan> = new Map();
  private modelDocumentation: Map<string, ModelDocumentation[]> = new Map();
   ...
   */
  registerModel(modelInfo: Omit<ModelRegistration, 'approvalDate' | 'nextReviewDate'>): string {
   ...
   */
  getValidationStrategy(modelId: string): ModelValidationStrategy | undefined {
   ...
   */
  setValidationStrategy(modelId: string, strategy: ModelValidationStrategy): void {
   ...
   */
  getModelDocumentation(modelId: string): ModelDocumentation[] {
   ...
   */
  addModelDocumentation(modelId: string, documentation: ModelDocumentation): void {
   ...
   */
  private createDefaultValidationStrategy(modelId: string, riskRating: string): void {
    const strategy: ModelValidationStrategy = {
      modelId,
   ...
   * @param riskRating Risk rating
   */
  private createDefaultMonitoringPlan(modelId: string, riskRating: string): void {
    const plan: ModelMonitoringPlan = {
      modelId,
      monitoringMetrics: [
        'MAE',
        'RMSE',
   ...
   */
  private createInitialDocumentation(modelId: string, modelInfo: any): void {
    const technicalDoc: ModelDocumentation = {
      modelId,
      version: '1.0',
...
${modelInfo.limitations.join('\n')}
    ...
    const governanceDoc: ModelDocumentation = {
      modelId,
      version: '1.0',
