...
import { CalculatorData, WealthProjection } from '../types/calculator';
...
export class AdvancedWealthCalculatorImpact {
  ...
  private static readonly ESTATE_TAX_EXEMPTION = 10000000; // ₹1 Crore (hypothetical for India)
  
  // Estate tax rates (hypothetical for India)
  private static readonly ESTATE_TAX_RATE = 0.10; // 10% flat rate
  
  // Legal and transfer costs as percentage of estate
  private static readonly TRANSFER_COST_PERCENTAGE = 0.05; // 5%
  
  // Education cost projections by type (current costs, will be inflated)
  private static readonly EDUCATION_COSTS: Record<string, number> = {
  ...
  private static readonly EDUCATION_INFLATION = 0.08; // 8% annually
  
  // Generational wealth transfer efficiency
  private static readonly GENERATIONAL_TRANSFER_EFFICIENCY: Record<string, number> = {
   ...
   */
  private static calculateInheritanceImpact(
    inputs: CalculatorData,
   ...
   * Calculate grandchildren impact
   */
  private static calculateGrandchildrenImpact(
    inputs: CalculatorData,
    inheritanceImpact: any,
    deathYear: number
  ): any {
   ...
   */
  private static calculatePlanningEfficiency(inputs: CalculatorData): number {
   ...
   */
  private static getAverageEducationPath(inputs: CalculatorData): string {
   ...
   */
  static calculateLifeEventImpacts(inputs: CalculatorData): Array<{
    event: string;
    timing: string;
