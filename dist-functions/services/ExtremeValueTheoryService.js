...
export class ExtremeValueTheoryService {
     ...
     */
    static fitGeneralizedParetoDistribution(data, thresholdPercentile = 0.95) {
     ...
     */
    static calculateExtremeVaR(model, confidenceLevel) {
     ...
     */
    static calculateExpectedShortfall(model, confidenceLevel) {
     ...
     */
    static performEVTAnalysis(data) {
     ...
     */
    static fitGPDByMethodOfMoments(exceedances) {
     ...
     */
    static calculateGoodnessOfFit(exceedances, parameters) {
     ...
     * @returns CDF value
     */
    static calculateGPDCDF(x, shape, scale) {
        if (x < 0)
            return 0;
        if (Math.abs(shape) < 1e-10) { // shape ≈ 0
     ...
     */
    static generateSyntheticLossData(returns) {
     ...
     */
    static getFallbackModel(data, threshold, thresholdPercentile) {
     ...
     */
    static assessTailRisk(var99, es99, model) {
        // Assess based on shape parameter (heavier tails = higher risk)
        const { shape } = model.parameters;
        if (shape > 0.3)
            return 'EXTREME'; // Very heavy tails
        if (shape > 0.1)
            return 'HIGH'; // Heavy tails
        if (shape > -0.1)
            return 'MEDIUM'; // Moderate tails
     ...
     */
    static generateTailEvents(model, var99, var995, var999) {
        return [
            {
